...
3.  **월별 요금 계산**: 배분된 사용량에 각 월의 평균열량과 열량단가를 곱하여 요금을 산출합니다.
   - 각 월의 경감액 적용 방식: 검침 주기(전월/당월) 일수 비율에 따라 해당 월의 경감액을 일할하여 합산합니다.
   - 경감액이 사용요금보다 클수는 없습니다.
    > **전월분 요금** = (`전월분 사용량` × `전월 평균열량` × `전월 열량단가`) - (`전월 경감액` x `전월 일수 비율`)
    > **당월분 요금** = (`당월분 사용량` × `당월 평균열량` × `당월 열량단가`) - (`당월 경감액` x `당월 일수 비율`)
    > **전월분 경감액** = min(`전월 경감액` × `전월 일수 비율`, `전월분 요금`) _# 경감액은 해당 월의 요금을 초과할 수 없음_
    > **당월분 경감액** = min(`당월 경감액` × `당월 일수 비율`, `당월분 요금`) _# 경감액은 해당 월의 요금을 초과할 수 없음_

4.  **최종 청구 요금**: 모든 요금을 합산하고, 각 월에 해당하는 경감액을 일할 적용한 뒤 부가가치세(10%)를 더합니다.
    > **최종 요금** = ( 기본요금 + 전월분 요금 - 전월분 경감액 + 당월분 요금 - 당월분 경감액 ) × 1.1
    - 계산 후 최종 요금은 **10원 이하 버림** 처리합니다.

5. ** 계산식 요약**: 
    > 사용요금 = (보정 사용량 × 전월 일수 비율 × 전월 평균열량 × 전월 열량단가)-(전월 경감액 x 전월 일수 비율) + (보정 사용량 × 당월 일수 비율 × 당월 평균열량 × 당월 열량단가)-(당월 경감액 x 당월 일수 비율)
    > **최종 요금** = ( 기본요금 + 사용요금  ) × 1.1
...
| **동절기 경감액** | 동절기(12~3월)에 적용되는 월별 경감액입니다. 일할계산 됩니다. |
| **동절기외 경감액** | 비동절기(4~11월)에 적용되는 월별 경감액입니다. 일할계산 됩니다. |
...
-   청구 요금 계산 방식 개선: 최종 금액을 10원 이하 버림 처리
