...
-   **다양한 요금제 지원**: '취사+난방' 혼합 요금제뿐만 아니라 '취사전용', '난방전용' 요금제까지 완벽하게 지원하여 실제 사용 패턴에 맞는 정확한 요금을 계산합니다.
-   **유연한 검침 주기 지원**: `매월` 검침뿐만 아니라, `격월(홀수/짝수월)` 또는 `3개월(분기별)` 정기 검침 주기를 완벽하게 지원합니다. 정기 주기 선택 시, 해당 기간의 사용량과 요금을 합산하여 보여주는 전용 센서들이 자동으로 추가됩니다.
-   **월패드 센서 지원**: 매월 1일 0으로 초기화되는 월패드형 가스 센서를 지원합니다. 옵션을 켜면 자동으로 누적 사용량으로 변환하여 요금을 계산합니다.
...
이 통합구성요소는 가스 사용량에 따른 **표준 요금**을 계산하는 것을 목표로 합니다. 
-   자동이체 할인, 전자고지서 할인
-   체납 가산금 또는 기타 비정기적 비용

* 센서에 표시되는 값과 실제 청구서의 최종 금액은 다를 수 있습니다.
...
| `chungcheong_gas` | 충청에너지서비스 (SK E&S) | 충북 | ✅ |
...
4.  설정 창에 아래 정보를 입력합니다.
    ...
    -   **사용 용도**: 본인의 요금제에 맞는 계산 방식을 선택합니다. (기본값: 취사+난방)
        -   `취사+난방 (경계값 사용)`: 취사와 난방을 함께 사용하는 일반적인 경우입니다.
        -   `취사전용`: 난방 없이 취사용으로만 가스를 사용하는 경우입니다.
        -   `난방전용`: 취사 없이 난방용으로만 가스를 사용하는 경우입니다.
    -   **난방 타입**: '주택난방(개별)'과 '중앙난방(공동)' 중 본인에게 해당하는 타입을 선택합니다. (선택한 공급사가 중앙난방을 지원하지 않는 경우 오류가 발생합니다.)
    -   **가스 사용량 센서 (m³)**: 현재 가스 누적 사용량을 나타내는 `sensor` 엔티티를 선택합니다. (예: `sensor.gas_meter`)
    -   **월패드 센서 사용 (매월 1일 초기화)**: 선택한 가스 센서가 누적 센서가 아니고 매월 1일 0으로 초기화되는 경우 체크합니다.
    ...
    -   **검침 주기**: 본인의 요금 청구 주기를 선택합니다. (기본값: 매월)
        -   `매월`
        -   `격월 - 홀수월 (1,3,5...)` / `격월 - 짝수월 (2,4,6...)`
        -   `3개월 - 1,4,7,10월` / `3개월 - 2,5,8,11월` / `3개월 - 3,6,9,12월`
...
| **취사난방경계** | `number.cooking_heating_boundary` | '취사+난방' 요금제에서만 사용되는 분리 기준(MJ). 기본값 0. 지원 공급사는 자동 설정되며, 필요시 수동 수정 가능. (보통 516MJ) |
| **동절기/비동절기 경감액** | `number.winter/non_winter_reduction_fee` | 계절에 따라 적용되는 월별 경감액 |

### `사용 용도` 설정에 따른 계산 분기

요금은 설정에서 선택한 '사용 용도'에 따라 아래와 같이 다르게 계산됩니다.

-   **취사전용**: 모든 사용량을 열량(MJ)으로 변환 후, 전월/당월 **취사 열량단가**만을 적용하여 요금을 계산합니다.
-   **난방전용**: 모든 사용량을 열량(MJ)으로 변환 후, 전월/당월 **난방 열량단가**만을 적용하여 요금을 계산합니다.
-   **취사+난방**: 아래의 상세 '계산 단계'에 따라 취사/난방 요금을 분리하여 정밀하게 계산합니다.
...
3.  **열량 변환 및 취사/난방 분리 (조건부)**
    > **참고**: 전월과 당월 모두 취사/난방 단가가 동일한 경우, 아래 분리 계산은 건너뛰고 모든 사용량은 난방용으로 간주됩니다.
...
| **전전월 사용 요금** | 2달 전 검침 주기의 최종 확정 요금입니다. |
| **마지막 동기화 성공** | 공급사 웹사이트에서 데이터를 성공적으로 가져온 마지막 시간입니다. |
| **가상 누적 가스 사용량** | (월패드 모드 선택 시) 매월 초기화되는 센서를 누적 값으로 변환한 센서입니다. |

#### 센서 (Sensor) - 정기 주기 전용 (격월/3개월)

설정에서 '검침 주기'를 `격월` 또는 `3개월`로 선택한 경우에만 아래의 센서들이 **추가로 생성**됩니다.

| 엔티티 이름 | 설명 |
| :--- | :--- |
| **정기 가스 사용량** | 현재 청구 주기(2달 또는 3달) 동안의 누적 가스 사용량입니다. |
| **정기 사용 요금** | 현재 청구 주기(2달 또는 3달) 동안의 누적 요금입니다. |
| **정기 예상 사용량** | 지난달(들)의 실제 사용량과 이번 달 예상 사용량을 합산한 총 예상 사용량입니다. |
| **정기 예상 요금** | 지난달(들)의 실제 요금과 이번 달 예상 요금을 합산한 총 예상 요금입니다. |
| **정기 직전 사용 요금**| 지난 정기 주기의 최종 확정 요금(2달치 또는 3달치 합산)입니다. |
...
| **월검침시작값** | 이번 달 요금 계산이 시작된 시점의 가스 계량기 값. 리셋 시, **(기존 시작값 + 청구된 정수 사용량)**으로 갱신되어, 청구되지 않은 소수점 이하 사용량을 다음 달로 이월시킵니다. |
...
| **취사난방경계** | 취사 요금을 적용할 월 최대 열량(MJ)입니다. **(기본값: 0, 지원 공급사는 자동으로 설정되며 0이면 모든 사용량에 난방 단가가 적용됩니다.)** |
...
| **기본요금 갱신** | 버튼을 누르면 즉시 공급사 웹사이트에서 최신 **기본요금** 및 **취사난방경계** 데이터를 가져옵니다. |
...
| `city_gas_bill.update_base_fee` | **'기본요금 갱신'** 버튼을 누르는 것과 동일한 작업을 수행합니다. 공급사 웹사이트에서 최신 **기본요금**과 **취사난방경계** 데이터를 가져와 업데이트합니다. 구성 등록변경시 자동실행 됩니다. |
...
-   **평균열량** 데이터는 설정된 **검침시간 5분 전**에 매일 자동으로 동기화됩니다.
...
-   사용자가 설정한 **'월 정기 검침일'과 '월 정기 검침시간'**이 동시에 일치할 때, 다음 작업을 수행합니다.
    ...
    2.  `월검침시작값`을 **(기존 시작값 + 청구된 정수 사용량)**으로 갱신하여, 청구되지 않은 소수점 이하 사용량을 다음 달로 정확히 이월시킵니다.
    3.  이후 새 주기의 요금/사용량 계산이 진행됩니다.

## 최근 변경 사항

-   **3개월 검침 주기 지원**: 분기별로 요금이 청구되는 사용자를 위해 3개월 합산 검침 주기 옵션을 추가했습니다.
-   **월패드 센서 지원**: 매월 1일 초기화되는 월패드 센서를 자동으로 누적 사용량으로 변환하는 기능을 추가하여 호환성을 높였습니다.
-   **리셋 로직 안정성 강화**: 센서 상태 변화 여부와 관계없이 설정된 검침 시간에 강제로 리셋 로직이 실행되도록 개선하여, 사용량이 없는 달에도 정확하게 주기가 갱신되도록 했습니다.
-   **요금 계산 방식 선택 기능 추가**: 설정에 '사용 용도' 옵션을 추가하여, '취사+난방', '취사전용', '난방전용' 등 실제 요금제에 맞춰 계산 방식을 직접 선택할 수 있도록 개선했습니다.
-   **신규 공급사 추가**: 삼천리, 대륜이엔에스, 참빛충북도시가스, 미래엔서해에너지, 충청에너지서비스 공급사를 추가했습니다.
-   **취사/난방 경계값 자동 설정**: 지원되는 공급사(삼천리 등)의 경우 '취사난방경계' 값을 최초 설정 시, 또는 '기본요금 갱신' 시 자동으로 가져와 업데이트하도록 개선했습니다.
-   **월 사용량 리셋 로직 수정 (중요)**: `월검침시작값`이 잘못 갱신되어 다음 달 사용량이 비정상적으로 계산되던 심각한 버그를 수정했습니다. 이제 청구되지 않은 소수점 이하 사용량이 다음 달로 정확하게 이월됩니다.
-   **요금 계산 로직 개선**: 취사 단가와 난방 단가가 모두 동일한 달에는 '취사난방경계' 값과 무관하게 단일 요금제로 계산하도록 최적화했습니다.
-   **설정 안정성 강화**: 검침시간 설정 시 'HH:MM' 형식과 'HH:MM:SS' 형식을 모두 지원하도록 파싱 로직을 개선했습니다.
