...
3.  **월별 요금 및 경감액 계산**: 배분된 사용량에 각 월의 요율을 곱하고, 해당 월의 경감액을 일할 적용합니다.
    - 전월이 동절기(12~3월)에 해당하면 '동절기 경감액'을, 아니면 '동절기외 경감액'을 사용합니다. 당월도 동일하게 적용됩니다.
    - **중요**: 일할 계산된 경감액은 해당 월의 사용요금을 초과할 수 없습니다.
    > **전월분 사용요금** = `전월분 사용량` × `전월 평균열량` × `전월 열량단가`
    > **당월분 사용요금** = `당월분 사용량` × `당월 평균열량` × `당월 열량단가`
    >
    > **전월분 적용 경감액** = min(`전월분 사용요금`, `해당월 경감액` × (`전월 일수` / `총 사용일수`))
    > **당월분 적용 경감액** = min(`당월분 사용요금`, `해당월 경감액` × (`당월 일수` / `총 사용일수`))

4.  **최종 청구 요금**: 모든 요금을 합산하고, 부가가치세(10%)를 더한 뒤, **10원 미만을 버립니다.**
