...
# [3. 임포트]
# 테스트할 공급사 클래스를 임포트합니다. 다른 공급사를 테스트하려면 이 부분을 수정하세요.
...
# --- 테스트할 지역 및 용도 설정 ---
TEST_REGION = "1"  # 예스코 기준 -> "1": 서울, "8": 경기
# 예스코는 주택난방만 지원하므로 'residential'로 고정합니다.
TEST_USAGE_TYPE = "residential" # "residential": 주택난방
# ---
...
async def main():
    ...
    logging.info(f"예스코 프로바이더 로컬 테스트를 시작합니다.")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        # 공급사 클래스를 초기화할 때 웹세션, 지역, 용도 정보를 전달합니다.
        provider = YescoGasProvider(
            websession=session, 
            region=TEST_REGION, 
